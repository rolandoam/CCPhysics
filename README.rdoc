...
== Really quick intro
	...
	- (id)init {
		...
		// start simulation - call [c stop] to stop the simulation
		[c start];
	}
...
For every CCPhysicsDynamicShape in the space, you need to provide an update method. Otherwise they would just fall because of (fake) gravity.
...
These shapes are defined that way because the basic "tile" is 32x32 (the grid for the spatial hash). Please notice that you can still add any kind of rectangle shape you want, given that they are AABB (axis aligned bounded box, that is, the "lines" of the box are aligned with the x and y axis).
...
Many, many thanks to the guys from whom I "got inspired" (read: ripped off) to create the engine.
